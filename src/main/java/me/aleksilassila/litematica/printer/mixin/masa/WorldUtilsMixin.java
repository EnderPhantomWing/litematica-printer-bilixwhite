...
public class WorldUtilsMixin {
    ...
    private static RayTraceUtils.RayTraceWrapper doSchematicWorldPickBlock(Level world, Entity dist2, double verifier, boolean posList, boolean traceMismatch, boolean worldClient, Operation<RayTraceUtils.RayTraceWrapper> original){
        if (USE_EASYPLACE.getBooleanValue() && Printer.getInstance().easyPos != null) {
            return new RayTraceUtils.RayTraceWrapper(RayTraceUtils.RayTraceWrapper.HitType.SCHEMATIC_BLOCK, new BlockHitResult(Vec3.atCenterOf(Printer.getInstance().easyPos), Direction.UP, Printer.getInstance().easyPos,false));
        }else {
